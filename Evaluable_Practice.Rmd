...
Since removing the outliers results in a degenerated variable, we  will remove it from the data, as it will
not provide any new information in order to classify the sale price.
...
## Classifier using linear discriminant analysis
...
distance<- get_dist(scale(numeric_data))
...
dendrogram <- hclust(dist(scale(numeric_data), method = 'euclidean'), method = 'ward.D')
...
k2 <- kmeans(scale(numeric_data), centers = 2, nstart = 25)
...
fviz_cluster(k2,data=numeric_data, ellipse.type = "norm")

```

```{r warning=FALSE, message=FALSE}

sil <- silhouette(k2$cluster, dist(scale(numeric_data)))
fviz_silhouette(sil)
...
set.seed(123)
fviz_nbclust(scale(numeric_data), kmeans, method = "silhouette")
...
set.seed(123)
gap_stat <- clusGap(scale(numeric_data), FUN = kmeans, nstart = 25,K.max = 10, B = 50)
...
The ARI value is 0.6116956. ARI is a measure of similarity between 2 cluster assignments (in this case, your PriceCategory variable and the clusters generated by K-means), adjusted by the possibility of random coincidence. ARI values can range from -1 to 1, where:
- ARI = 1 indicates a perfect match between the two groupings.
- ARI = 0 indicates a match no better than random chance.
- ARI < 0 indicates a worse match than random chance.
In this case, an approximated value of 0.611 suggests that there's a  match between the clusters generated by K-means and the PriceCategory variable, better than random chance. 