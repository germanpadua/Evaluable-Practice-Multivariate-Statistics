...
data_train$PriceCategory <- ntile(data_train$SalePrice, 2)

# Asignar nombres a las categorías
data_train$PriceCategory <- ifelse(data_train$PriceCategory == 1, "Cheap", "Expensive")
...
data_train$PriceCategory<-as.factor(data_train$PriceCategory)


set.seed(3)

# Partitioning the dataset: training (80%) + test (20%)
trainIndex<-createDataPartition(data_train$PriceCategory,p=0.80)$Resample
datos_train<-data_train[trainIndex,]
datos_test<-data_train[-trainIndex,]
...
distance<- get_dist(numeric_data_train)
...
dendrogram <- hclust(dist(numeric_data_train, method = 'euclidean'), method = 'ward.D')
...
k2 <- kmeans(numeric_data_train, centers = 2, nstart = 25)
...
fviz_cluster(k2,data=numeric_data_train)
...
set.seed(123)
fviz_nbclust(numeric_data_train, kmeans, method = "wss")
...
data_train$Cluster <- k2$cluster

# Crear una tabla cruzada para comparar los clusters con PriceCategory
table(data_train$PriceCategory, data_train$Cluster)
...
adjusted_rand_index <- adjustedRandIndex(data_train$PriceCategory, data_train$Cluster)
